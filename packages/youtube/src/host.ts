 ...
 * Comlink delivers messages in order, so the `connect` sent here arrives before
 * any call the caller makes with the returned client.
 */
export function connectEngine(worker: Endpoint, fetch: FetchLike): EngineClient {
  ...
  void remote.connect(proxy(host));
  return remote;
}
