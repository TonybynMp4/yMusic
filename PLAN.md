...
The Sign in button opens a menu: Google's page as above, or importing the session from a browser that is already signed in. Firefox and its forks store cookies in the clear. Chromium browsers encrypt them; on Linux the key is in the Secret Service under "<Browser> Safe Storage", so Chrome, Chromium, Brave, Edge, Vivaldi and Opera work there. On Windows, Chromium uses app-bound encryption that only the browser can undo, so Windows offers Firefox-family browsers only. An imported session stays shared with the browser, so signing out there signs out here too.
